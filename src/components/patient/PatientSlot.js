...
//           ? response.data.data
//               .filter((appointment) => appointment.is_booked)
//               .map((appointment) => ({
//                 ...appointment,
...
//         // Map through response to add status based on is_canceled and is_complete
...
const PatientSlot = () => {
  ...
  const togglePrescriptionForm = async (appointment_id) => {
    ...
    if (!showPrescriptionForm) {
        ...
        if (selectedAppointment) {
          ...
          if (fetchDataResponse.status === 200) {
            ...
            setPrescriptionDetails({
              patient_id: patient_id,
              appointment_id: appointment_id,
              medicine_name: fetchedData.medicine_name || "",
      ...
      setPrescriptionDetails({
        patient_id: formDetails.patient_id,
        medicine_name: "",
        time: "",
        comment: "",
        description: "",
  ...
  const toggleFormModal = async () => {
    ...
    if (!showFormModal) {
      ...
      if (
        ...
        formDataToSend.append("document_file", selectedFiles[0]);
        formDataToSend.append("user_type", userType);
        formDataToSend.append("user_id", patientId);
        try {
          setLoading(true);
          const postResponse = await BaseUrl.post(
            "/patient/patientdocumentusingappointmentid/",
  ...
  return (
    <div
      className="container-fluid"
      style={{ backgroundColor: "#D7EAF0", overflowX: "auto" }}
    >
      {loading && (
        <LoaderWrapper>
          <LoaderImage>
