...
//       const endpoint = `/clinic/countavailableslots/?doctor_id=59&${datesQuery}`;
...
//         await BaseUrl.get(
//           `/clinic/availableslots/?doctor_id=59&date=${date}`
//         );
//       }
//     } catch (error) {
//       console.error(error);
...
//     try {
//       const endpoint = `/doctorappointment/blankslot/?doctor_id=59&slot_date=${selectedDate}&consultation_type=${appointmentType}`;
...
//           `/doctorappointment/blankslot/?doctor_id=59&slot_date=${selectedDate}&consultation_type=${type}`
//         );
...
//         currency: "INR",
//         // customer_name: name,
//         customer_phone: extractedMobileNumber,
...
//         paymentSessionId,
//         returnUrl: "http://localhost:3001/patientbook",
//       });
...
//       }));
//       for (let date of selectedDates) {
//         await BaseUrl.get(`/clinic/availableslots/?doctor_id=59&date=${date}`);
//       }
//     } catch (error) {
//       console.error(error);
//     }
...
//   //     navigate("/components/checkout"); // Navigate to the Checkout route
...
//   //     // Store the country code in localStorage
...
//                 CLINIC VISIT
//               </Card.Title>
//               <Card.Text style={{ fontSize: "12px", marginBottom: "5px" }}>
//                 Book Physical Appointment
//               </Card.Text>
//               <Card.Text
...
//                 }}
//               >
//                 Consultancy Fee: {consultancyFee}
//               </Card.Text>
//               <div>
//                 <Form.Select
//                   style={{
...
//                       // Sync country selection based on phone number
//                       if (value) {
//                         const matchedCountry = countries.find((c) =>
//                           value.startsWith(c.code)
//                         );
//                         if (matchedCountry) {
//                           setSelectedCountry(matchedCountry.name); // Update dropdown
//                           setConsultancyFee(matchedCountry.fee); // Update fee
//                         }
//                       }
//                     }}
//                     defaultCountry="IN"
...
//               <Button
//                 variant="success"
//                 onClick={handleSubmit} // Call navigate function directly
//                 disabled={
//                   loading ||
//                   !name ||
//                   !mobile_number ||
...
const BookAppointment = () => {
  ...
  const fetchAvailableSlotsCount = async (selectedDates) => {
    try {
      ...
      for (let date of selectedDates) {
        await BaseUrl.get(`/clinic/availableslots/?doctor_id=2&date=${date}`);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSlotClick = async (slot, selectedDate, type) => {
    try {
      ...
      if (updateResponse.status === 200) {
        const fetchResponse = await BaseUrl.get(
          `/doctorappointment/blankslot/?doctor_id=2&slot_date=${selectedDate}&consultation_type=${type}`
        );
