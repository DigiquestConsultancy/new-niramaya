...
const ReceptionHome = () => {
  ...
  const fetchAppointmentsData = useCallback(
    async (doctorId, receptionId, date) => {
      if (receptionId) {
        try {
          const response = await BaseUrl.get(
            `/reception/allappointments/?reception_id=${receptionId}&appointment_date=${date}`
          );
  ...
  //           }}
  //         >
  //           {/* Check if the document has a previewable file (image, PDF thumbnail, etc.) */}
  //           {document.document_file ? (
  //             <img
  //               src={`Uploaded by: ${document.document_file}`} // assuming document_file is the image URL
  //               alt={`Uploaded by: ${document.uploaded_by}`} // Format the alt text
  //               style={{
  //                 objectFit: "cover",
  //                 width: "100%",
  //                 height: "90%",
  ...
  //                 cursor: "pointer",
  //               }}
  //               onClick={() => viewDocument(document.id)} // Add onClick to view the document
  //             />
  //           ) : (
  //             <span>No Preview</span> // Handle non-previewable documents
  //           )}
  //         </div>

  //         {/* Document Actions (Modify/Delete) */}
      ...
      <header className="mb-2 reception-header d-flex flex-column flex-md-row justify-content-between align-items-center text-center text-md-start">
        ...
        <Col xs={12} md="auto" className="flex-grow-1">
          <h1 className="text-center" style={{ color: "#0174BE" }}>
            Welcome to Niramaya Homeopathy Reception
          </h1>
        </Col>
        <Col xs={12} md="auto">
          <img
      ...
      <Row className="mb-4">
        <Col md={4}>
          <Card.Body>
            ...
            <Card.Text>
              Explore our reception services and treatment offered.
            </Card.Text>
            <Button as={Link} to="/reception/" variant="primary">
              Explore reception
            </Button>
