...
const ReceptionAddSlot = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async (receptionId) => {
      try {
        ...
        if (response.data.doctor_id) {
          setDoctor(response.data); // Set the doctor object
        } else {
          setErrorMessage('Unexpected data format from the API');
        }
    ...
    if (token) {
      try {
        ...
        const receptionId = decodedToken.reception_id; // Assuming reception_id is stored in the token
        fetchDoctors(receptionId); // Fetch doctor details using reception_id from token
      } catch (error) {
        ...
        <form className="p-4 shadow" style={{ backgroundColor: '#f9f9f9', borderRadius: '8px' }} onSubmit={handleSubmit}>
          ...
          <div className="form-row mb-3">
            ...
            <div className="form-group col-md-4">
              ...
              <select className="form-control" name="interval_minutes" value={formData.interval_minutes} onChange={handleChange} required>
...
export default ReceptionAddSlot;
