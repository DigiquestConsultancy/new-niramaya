...
const DoctorLogin = ({ setIsDoctorLoggedIn }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      if (forgotPassword) {
        const response = await BaseUrl.get(`/doctor/userverification/?mobile_number=${mobileNumber}`);
        if (response.status === 200) {
          setShowVerification(true); // Show OTP verification form
          setMessage({ type: 'success', text: 'OTP sent to your mobile number.' });
        }
        ...
        if (response.status === 200) {
          ...
          if (isValidToken(token)) {
            ...
            if ((userType === 'clinic' || userType === 'reception') && !isReset) {
        ...
        <div className="col-md-12 col-lg-6 d-flex justify-content-center justify-content-lg-end align-items-center form-container mt-4">
          ...
          {createNewPasswordModal ? (
            ...
            <form className="login-form log mb-4" onSubmit={handleLogin}>
              <div className="doctor-login-link">
                <p className='text-link' >Are you a Patient? <Link to="/patient/login">Login here</Link></p>
              </div>
              <h2 className="text-dark mb-4">Doctor Login</h2>
              ...
              <div className="d-flex justify-content-between align-items-center mb-4">
                <div className="form-check">
                  <input
                    ...
                    onChange={(e) => setLoginWithOtp(e.target.checked)}
                  />
                  <label className="form-check-label" htmlFor="Check">Login with OTP instead of Password</label>
                </div>
 
                <button type="button" className="btn btn-link p-0" onClick={() => setForgotPassword(true)}><b>Forgot Password?</b></button>
              </div>
 
              <button type="submit" className="btn btn-primary w-45 mb-3">{loginWithOtp ? 'SEND OTP' : 'LOGIN'}</button>
            ...
            <form className="forgot-password-form mb-4" onSubmit={handleLogin}>
              ...
              <button type="submit" className="btn btn-primary w-25 mb-3">SEND OTP</button>
              <button
                type="button"
                className="btn d-flex align-items-center justify-content-start"
                onClick={() => setForgotPassword(false)}
