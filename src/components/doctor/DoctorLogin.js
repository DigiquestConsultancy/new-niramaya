...
//           if (
//             (user_type === "clinic" || user_type === "reception") &&
//             !is_reset
//           ) {
...
//             }}
//             style={{
//               color: activeTab === "login" ? "orange" : "#007bff", // Active tab is orange
//               cursor: "pointer",
//               marginRight: "10px",
...
//             }}
//             style={{
//               color: activeTab === "register" ? "orange" : "#007bff", // Active tab is orange
//               cursor: "pointer",
//               marginLeft: "10px",
...
const DoctorLogin = ({ setIsDoctorLoggedIn }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (loginWithOtp) {
        response = await BaseUrl.get(`/doctor/login/`, {
          params: { mobile_number: mobileNumber },
        });
        if (response.status === 200 && response.data.success) {
          setShowVerification(true);
  ...
  const handleResendOTP = async () => {
    try {
      const response = await BaseUrl.get(`/doctor/login/`, {
        params: { mobile_number: mobileNumber },
      });
      if (response.status === 200 && response.data.success) {
        setMessage({ type: "success", text: response.data.success });
              ...
              <form className="otp-form" onSubmit={handleVerifyOTP}>
                ...
                <div className="d-flex justify-content-between">
                  <button
                    ...
                    className="btn btn-secondary"
                    onClick={handleResendOTP}
                    disabled={false} // Add logic to disable after certain time
                  >
                    Resend OTP
                  </button>
                  <button type="submit" className="btn btn-primary">
