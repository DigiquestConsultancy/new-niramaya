...
const DoctorDetails = () => {
  ...
  const [personalDetailsSuccessMessage, setPersonalDetailsSuccessMessage] = useState("");
const [personalDetailsErrorMessage, setPersonalDetailsErrorMessage] = useState("");

  const [addressSuccessMessage, setAddressSuccessMessage] = useState("");
const [addressErrorMessage, setAddressErrorMessage] = useState("");

  const [opdDetailsSuccessMessage, setOpdDetailsSuccessMessage] = useState("");
  const [opdDetailsErrorMessage, setOpdDetailsErrorMessage] = useState("");
  const [timingSuccessMessage, setTimingSuccessMessage] = useState("");
  const [timingErrorMessage, setTimingErrorMessage] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setPersonalDetailsSuccessMessage(response.data.success);
        setTimeout(() => setPersonalDetailsSuccessMessage(""), 3000);
        setIsExistingUser(true);
        setIsPersonalComplete(true);
        await updateProgress();
        await updateQualifications(doctor_id);
      ...
      setPersonalDetailsErrorMessage("Failed to save personal details. Please try again.");
      // Automatically hide after 3 seconds
      setTimeout(() => setPersonalDetailsErrorMessage(""), 3000);
    } finally {
      setLoading(false);
    }
  ...
  //       if (formData.qualification.includes(qual.value)) {
  //         if (typeof qual.value === "number") {
  //           existing.push({ id: qual.value, is_selected: true });
  //         } else {
  //           newOnes.push(qual.label);
  //         }
  //       }
  //     });

  //     // First, send new qualifications to the backend (assuming POST creates them and returns their IDs)
  //     const newQualificationIds = [];
  //     for (const label of newOnes) {
  //       const res = await BaseUrl.post("/doctor/qualification/", {
  //         qualification: label,
  //       });
  //       if (res.data && res.data.id) {
  //         newQualificationIds.push({ id: res.data.id, is_selected: true });
  //       }
  //     }

  //     const finalQualifications = [...existing, ...newQualificationIds];
  ...
  //     if (response.status === 200) {
  //       await fetchQualifications(); // Refresh dropdown
  //     }
  //   } catch (error) {
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setAddressSuccessMessage(response.data.success || "");
      ...
      if (error.response?.data?.error) {
        setAddressErrorMessage(error.response.data.error);
      } else {
        setAddressErrorMessage("An error occurred. Please try again.");
      }
      setTimeout(() => setAddressErrorMessage(""), 3000);
    } finally {
      setLoading(false);
    }
  ...
  const handleOpdSubmit = async (e) => {
    setLoadingOpd(true);
    setOpdDetailsSuccessMessage(""); // Reset success message
    setOpdDetailsErrorMessage(""); // Reset error message
    e.preventDefault();
  
    const errors = {};
    
    // Check if the start day is greater than the end day
    const startDayIndex = getDayIndex(opdData.start_day);
    const endDayIndex = getDayIndex(opdData.end_day);
  
    if (startDayIndex > endDayIndex) {
      errors.start_end_day = ["Start Day cannot be later than End Day."];
    }
  
    if (!opdData.clinic_name) errors.clinic_name = ["Clinic Name is required."];
    if (!opdData.start_day) errors.start_day = ["Start Day is required."];
    if (!opdData.end_day) errors.end_day = ["End Day is required."];
    if (!opdData.consultation_fee) errors.consultation_fee = ["Consultation fee is required."];
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setOpdDetailsSuccessMessage(response.data.success || "");
        setTimeout(() => setOpdDetailsSuccessMessage(""), 3000);
        fetchOpdDetails();
        if (!opdId) {
          setOpdId(response.data.id);
        }
      ...
      setOpdDetailsErrorMessage(error.response?.data?.error || "");
      setTimeout(() => setOpdDetailsErrorMessage(""), 3000);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveTimings = async () => {
    setLoadingTimings(true);
    setTimingSuccessMessage(""); // Reset success message
    setTimingErrorMessage(""); // Reset error message
    ...
    try {
      ...
      responses.forEach((response) => {
        setTimingSuccessMessage(response.data.success);
        setTimeout(() => setTimingSuccessMessage(""), 3000);
      });

    } catch (error) {
      setTimingErrorMessage("Failed to save OPD timings. Please try again.");
      setTimeout(() => setTimingErrorMessage(""), 3000);
    } finally {
      setLoadingTimings(false);
    }
  };
  ...
  onChange={(selectedOptions) => {
    const updatedOptions = selectedOptions.map((option) => {
      // If custom (no ID), generate a UUID-like placeholder
      if (!option.value.toString().match(/^\d+$/)) {
        return {
          value: option.value,
          label: option.label,
          is_new: true, // Mark as custom
        };
      }
      return option;
    });
    ...
    setFormData((prevFormData) => ({
      ...prevFormData,
      qualification: updatedOptions.map((opt) => opt.value),
    }));

    // Add any new custom ones to the options list so they persist in dropdown
    const newOptions = updatedOptions.filter(
  (opt) =>
    !qualifications.find((q) => q.value === opt.value) &&
    !opt.value?.toString().match(/^\d+$/)
);


    if (newOptions.length > 0) {
      setQualifications((prev) => [...prev, ...newOptions]);
    }
  }}
  className={formErrors.qualification ? "is-invalid" : ""}
/> */}
    ...
    <div className="message-container" style={{ flex: 1, textAlign: "center" }}>
      {personalDetailsSuccessMessage && (
        <p className="text-success" style={{ zIndex: 9999 }}>
          {personalDetailsSuccessMessage}
        </p>
      )}
      {personalDetailsErrorMessage && (
        <p className="text-danger" style={{ zIndex: 9999 }}>
          {personalDetailsErrorMessage}
        </p>
      )}
    </div>
      ...
      <div className="row mb-3">
        ...
        <div className="col-md-4">
          ...
          <input
            ...
            maxLength={6} // Optional: Limit to 6 digits (Indian pincode)
            onChange={(e) => {
              const value = e.target.value;
              if (/^\d*$/.test(value)) {
                handleAddressChange(e);
        ...
        <div className="col-md-6">
          ...
          <input
            ...
            onChange={(e) => {
              const value = e.target.value;
              if (/^\d*$/.test(value)) {
                handleOpdChange("consultation_fee", e);
              }
            }}
          />
          {formErrors.opd?.consultation_fee && (
  ...
  <span className="message-container" style={{ flex: 1, textAlign: "center" }}>
    {opdDetailsSuccessMessage && (
      <p className="text-success" style={{ zIndex: 9999 }}>
        {opdDetailsSuccessMessage}
      </p>
    )}
    {opdDetailsErrorMessage && (
      <p className="text-danger" style={{ zIndex: 9999 }}>
        {opdDetailsErrorMessage}
      </p>
    )}
