...
//         const responseSlots = await BaseUrl.get(`/doctorappointment/slot/`, {
//           params: {
//             doctor_id: doctorId,
...
//         fetchAppointmentCounts();
//       } else {
//         setErrorMessage("Failed to cancel the appointment.");
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 p-4 overflow-y-auto">
        ...
        <div className="w-100 d-flex justify-content-center align-items-center flex-nowrap">
          <button
          ...
          <h5 className="text-center mx-2 my-0" style={{ minWidth: "120px" }}>
            {formattedDate}
          </h5>
          <button
            className="btn btn-outline-primary mx-2"
      ...
      `}</style>
          ...
          {showSlots && (
            <>
              <div
                className="d-flex flex-wrap justify-content-center align-items-center border rounded shadow-sm mx-auto text-sm "
                style={{
                  width: "fit-content",
                  fontSize: "1.2rem",
                  margin: "10px auto",
              ...
              <Row className="p-3 d-lg-flex d-md-flex d-grid ">
                <Col className="">
                  <h5 className="text-center mb-3">Morning</h5>
                  {morningSlots.length > 0 ? (
                    renderSlotCards(morningSlots)
...
//     // Function to parse multiple field-value pairs
//     const parseFieldsAndValues = (text, mappings) => {
//       const updates = {};
//       let remainingText = text;

//       for (const [spokenField, stateField] of Object.entries(mappings)) {
//         if (remainingText.includes(spokenField)) {
//           const fieldStart = remainingText.indexOf(spokenField);
//           const fieldEnd = fieldStart + spokenField.length;
//           let valueStart = fieldEnd;

//           let nextFieldStart = remainingText.length;
//           for (const [nextField] of Object.entries(mappings)) {
//             const pos = remainingText.indexOf(nextField, fieldEnd);
//             if (pos !== -1 && pos < nextFieldStart) {
//               nextFieldStart = pos;
//             }
//           }

//           const value = remainingText.substring(valueStart, nextFieldStart).trim();
//           if (value) {
//             updates[stateField] = value;
//           }

//           remainingText = remainingText.substring(nextFieldStart).trim();
//         }
//       }
...
//     if (type === 'vitals') {
//       const updates = parseFieldsAndValues(transcript, fieldMappings.vitals);
...
//         }));
//       } else {
//         setVitals(transcript); // Fallback
//       }
//     } else if (type === 'symptoms') {
//       const updates = parseFieldsAndValues(transcript, fieldMappings.symptoms);
//       if (Object.keys(updates).length > 0) {
//         setNewSymptom((prev) => ({
//           ...prev,
//           ...updates,
//         }));
//         setShowAddSymptomRow(true); // Show the symptom row if not already visible
//       } else {
//         setSymptoms(transcript); // Fallback
//       }
//     } else if (type === 'prescriptions') {
//       const updates = parseFieldsAndValues(transcript, fieldMappings.prescriptions);
//       if (Object.keys(updates).length > 0) {
//         setFormPrescription((prev) => ({
//           ...prev,
//           ...updates,
//         }));
//       } else {
//         setPrescriptions(transcript); // Fallback
//       }
//     }
//   };
...
//       // Only restart if explicitly listening and not already restarting
//       if (isListening) {
//         setTimeout(() => {
//           try {
//             recognition.start();
...
//   recognition.onend = () => {
//     // Only restart if explicitly requested to keep listening
//     if (isListening) {
...
//                 stopVoiceTyping();
//               } else {
//                 startVoiceTyping("prescriptions");
//               }
//             }}
...
//   />
//   <button onClick={() => startVoiceTyping('symptoms')}>Start Voice Typing</button>
//   <button onClick={stopVoiceTyping}>Stop Voice Typing</button>
// </div> */}
