...
import logon from "../../images/logon.jpeg";
...
const DoctorHome = () => {
  ...
  const [clinicName, setClinicName] = useState("Niramaya Homeopathy");
  ...
  const fetchClinicDetails = async () => {
    try {
      const response = await BaseUrl.get(`/doctor/opddays/`, {
        params: {
          doctor_id: doctorId,
          mobile_number: mobileNumber, // Passing mobile number in the request
        },
      });
      ...
      const response = await BaseUrl.get(`/reception/walkincount/`,
        {
          params: {
            ...
            // patient_name: selectedAppointment ? selectedAppointment.booked_by : "",
            // patient_mobile: selectedAppointment ? selectedAppointment.mobile_number : "",
          },
        }
      );
 
      if (response.status === 200) {
        const data = response.data.reduce((acc, curr) => ({ ...acc, ...curr }), {});
  ...
  const fetchAppointments = useCallback(async () => {
    try {
      ...
      // Fetch today's appointments, completed appointments, and canceled appointments
      ...
      const responseSlots = await BaseUrl.get(`/doctorappointment/todayslot/?doctor_id=${doctorId}`,
        {
          params: {
            doctor_id: doctorId,
            appointment_date: formattedISODate,
  ...
  const handleSearch = async () => {
    try {
      const doctorId = jwtDecode(localStorage.getItem("token")).doctor_id;
      if (!doctorId) return;
      ...
      rows.push(
        <Row key={i} className="mb-2">
          {slotChunk.map((slot, index) => {
            ...
            if (slot.is_booked) {
              cardStyle = {
                backgroundColor: "#D1E9F6", // Blue for booked slots
                color: "#000",
              };
            } else if (slot.is_blocked) {
              cardStyle = {
                backgroundColor: "#FFA38F", // Orange for blocked slots
                color: "#000",
              };
            } else {
              cardStyle = {
                backgroundColor: "#A2CA71", // Green for available slots
                color: "#000",
              };
            }
  ...
  const renderAppointments = (appointments, index, setIndex, type) => {
    const endIndex = Math.min(index + 4, appointments.length);
    const displayedAppointments = appointments.slice(index, endIndex);
    ...
    return (
      <Row className="mb-4 text-center align-items-center justify-content-center">
        <Col xs="auto">
          <Button
            variant="outline-primary"
            onClick={() => setIndex((prevIndex) => Math.max(prevIndex - 1, 0))}
            disabled={index === 0}
          >
            <BsChevronLeft />
          </Button>
        ...
        {displayedAppointments.length > 0 ? (
          displayedAppointments.map((appointment, idx) => {
            const isSelected = selectedAppointment === appointment; // Check if this appointment is selected
            return (
              <Col key={idx}>
                <Card
                  className={`mb-4 shadow-sm doctor-card ${isSelected ? 'selected' : ''}`}
                  onClick={() => handleAppointmentClick(appointment, type)}
                  style={{
                    borderColor: isSelected ? '#007bff' : '',
                    backgroundColor: isSelected ? '#e9f7fe' : '',
                  }}
                >
                  <Card.Body>
          ...
          <Col xs="auto" className="d-flex justify-content-center">
            <div
              ...
              role="alert"
            >
              {`No ${type} appointments available.`}
            </div>
          </Col>
        )}
        <Col xs="auto">
          <Button
            variant="outline-primary"
            onClick={() =>
              setIndex((prevIndex) => Math.min(prevIndex + 1, appointments.length - 4))
            }
            disabled={index >= appointments.length - 4}
          >
            <BsChevronRight />
          </Button>
        </Col>
  ...
  return (
    <div className="doctor-container">
       <header className="header">
        <img src={clinicPhoto} alt="Clinic Logo" className="logo" />
        <h1 className="heading">Welcome to {clinicName}</h1>
      </header>
      ...
      <Row className="mb-4 text-center">
        <Col>
          <Card.Body>
            ...
            <Card.Text style={{ paddingTop: '10px', fontSize: '22px' }}>{totalAppointments}</Card.Text>
          </Card.Body>
        </Col>
        ...
        <div className="d-flex justify-content-end p-2">
          <div className="search d-flex align-items-center">
            <input
              style={{ width: '500px' }}
              type="text"
              placeholder="Search (Doctor Name, Mobile Number, Patient Name, Appointment Date)"
              value={searchQuery}
      ...
      <Row className="mb-4 text-center align-items-center justify-content-center appointment-list">
        ...
        {todayAppointments.length > 0 ? (
          renderAppointments(todayAppointments, currentIndex, setCurrentIndex, "today")
        ) : (
          <Col xs="auto" className="d-flex justify-content-center">
            <div
              className="alert alert-danger p-2"
        ...
        {todayAppointments.length > 4 && currentIndex < todayAppointments.length - 4 && (
          <Col xs="auto">
      ...
      {renderAppointments(completedAppointments, completedIndex, setCompletedIndex, "completed")}
 
      <h3 className="text-center">Canceled Appointments</h3>
      {renderAppointments(canceledAppointments, canceledIndex, setCanceledIndex, "canceled")}
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          {selectedAppointment && (
            <>
              <p><strong>Appointment Date:</strong> {format(new Date(selectedAppointment.appointment_date), "dd/MM/yyyy")}</p>
              <p><strong>Appointment Slot:</strong> {selectedAppointment.appointment_slot}</p>
              <p><strong>Booked By:</strong> {selectedAppointment.booked_by}</p>
              <p><strong>Doctor Name:</strong> {selectedAppointment.doctor_name}</p>
              <p><strong>Mobile Number:</strong> {selectedAppointment.mobile_number}</p>
            </>
          )}
        </Modal.Body>
        <Row className="px-3 pb-3">
          <Col>
            <span style={{ marginRight: "10px" }}>End Visit:</span>
            <div
              onClick={() => handleEndVisit(selectedAppointment.appointment_id)}
              style={{
                position: "relative",
                width: "40px",
                height: "20px",
                backgroundColor: isEnded ? "#2196F3" : "#ccc",
                borderRadius: "20px",
                cursor: "pointer",
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  top: "1px",
                  left: isEnded ? "21px" : "1px",
                  transition: "transform 0.4s",
                  transform: isEnded ? "translateX(-1px)" : "translateX(0)",
                }}
              ></div>
            </div>
          </Col>
          ...
          <Col>
            <span style={{ marginRight: "10px" }}>Cancel Appointment:</span>
            <div
              ...
              style={{
                ...
                width: "40px",
                height: "20px",
                backgroundColor: isCanceled ? "#f44336" : "#ccc",
                borderRadius: "20px",
                cursor: "pointer",
                transition: "background-color 0.4s",
              }}
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  top: "1px",
                  left: isCanceled ? "21px" : "1px",
                  transition: "transform 0.4s",
                  transform: isCanceled ? "translateX(-1px)" : "translateX(0)",
                }}
              ></div>
            </div>
          </Col>
      ...
      {/* Modal for Completed Appointment without toggle buttons */}
      <Modal show={showCompletedModal} onHide={() => setShowCompletedModal(false)}>
        <Modal.Header closeButton>
      ...
      <Modal show={showCanceledModal} onHide={() => setShowCanceledModal(false)}>
        <Modal.Header closeButton>
