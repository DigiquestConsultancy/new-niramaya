...
//     if (items.length === 0 || items.some((item) => !item.description || item.qty <= 0 || item.amount < 0)) {
//       setError("Please add at least one valid service with description, quantity, and amount.");
...
//                             value={item.amount}
//                             onChange={(e) =>
//                               handleItemChange(index, "amount", parseInt(e.target.value) || 0)
//                             }
//                             min="0"
//                           />
//                         </div>
...
//                             value={item.discount}
//                             onChange={(e) =>
//                               handleItemChange(index, "discount", parseInt(e.target.value) || 0)
//                             }
//                             min="0"
//                           />
//                         </div>
...
const GenerateInvoice = ({ isOpen, onCloseInvoice, onCreate, appointment }) => {
  ...
  const fetchPdf = async (pdfUrl) => {
    try {
      const token = localStorage.getItem("token");
      const response = await BaseUrl.get(pdfUrl, {
        responseType: "blob",
        headers: {
      ...
      setPdfError(`Failed to load PDF: ${error.response?.statusText || "Unknown error"}`);
  ...
  const handleCreateReceipt = async () => {
    ...
    const payload = {
      appointment_id: appointment?.appointment_id,
      patient: 321, // Hardcoded as per previous code
      doctor_id: doctorId,
      name: patient.name,
      phone: patient.phone,
      uh_id: patient.uhid,
    ...
    try {
      ...
      const getResponse = await BaseUrl.get("/doctor/receipt/", {
        params: {
          patient_id: 321, // Hardcoded as per previous code
          appointment_id: appointment?.appointment_id,
        },
      });

      if (getResponse.data.length > 0) {
        const firstReceipt = getResponse.data[0];
        const blobUrl = await fetchPdf(firstReceipt.pdf_url);
        if (blobUrl) {
  ...
  return (
    <div
      className="modal fade show d-block"
      tabIndex="-1"
      style={{ backgroundColor: "rgba(0,0,0,0.5)", display: isOpen ? "block" : "none" }}
    >
      <style>
        {`
          @media (max-width: 576px) {
