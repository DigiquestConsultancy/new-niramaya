...
const ClinicDetails = () => {
  ...
  const [successMessage, setSuccessMessage] = useState(""); // Display success message from backend
    ...
    try {
      ...
      if (response.status === 200) {
        if (response.data.success) {
          setSuccessMessage(response.data.success); // Set success message from backend
        }
  ...
  const handleChange = (e) => {
    ...
    if (
      ...
      if (!alphabetAndDotOnly.test(value)) return;
    }
    if (name === "age") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setErrorMessage(response.data.error || "Failed to update details.");
        setSuccessMessage("");
        setShowModal(true);
      }
    } catch (error) {
        ...
        <div className="row mb-4">
          ...
          <div className="col-md-4">
            ...
            <input
              ...
              className="form-control"
              name="mobile_number"
              value={formData.mobile_number} // Display mobile number from backend
              onChange={handleChange}
              required
            />
          </div>
