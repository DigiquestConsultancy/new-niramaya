...
const ClinicAddSlot = () => {
  ...
  const handleViewAllSlots = () => {
    if (selectedDoctor) {
      ...
      setIsViewAll(true); // Mark view all as active
    } else {
      setErrorMessage("Please select a doctor to view all slots.");
    }
  };
  ...
  const handleDateRangeChange = (direction) => {
    if (direction === "prev" && currentDateIndex > 0) {
      ...
      fetchSlots(
        selectedDoctor,
        format(parse(dates[currentDateIndex - 1], "dd/MM/yyyy", new Date()), "yyyy-MM-dd")
      );
    } else if (
      direction === "next" &&
      currentDateIndex < dates.length - dateRangeSize
          ...
          .map((date, index) => (
            <button
              ...
              onClick={() => {
                ...
                setIsViewAll(false); // Disable "View All" mode when selecting a date
              }}
            >
              {date}
            </button>
