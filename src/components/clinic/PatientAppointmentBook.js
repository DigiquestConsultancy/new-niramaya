...
// Define Loader components using styled-components
...
const PatientAppointmentBook = () => {
    ...
    const fetchDoctorName = async (clinicId) => {
        try {
            ...
            setErrorMessage(error.response?.data?.error || 'An error occurred while fetching doctor name.');
        } finally {
    ...
    const handleSaveDetails = async (e) => {
        ...
        try {
            ...
            } else if (response.status === 400 && response.data.error === "Patient with this mobile number already exists.") {
    ...
    const handleViewSlots = async (date) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setShowSlots(true);
                setSlotErrorMessage(response.data.length === 0 ? 'No slots available for the selected date.' : '');
                setErrorMessage('');
            } else {
    ...
    const handleConfirmAppointment = async () => {
        try {
            // First, book the appointment using POST request
            setLoading(true); // Start loading
    ...
    const handleSelectPatient = async (patient) => {
        try {
            setLoading(true); // Start loading
            const response = await BaseUrl.get(`/patient/patient/?appointment_id=${patient.appointment}&patient_id=${patient.patient}`);
            if (response.status === 200) {
    ...
    return (
        <div className="container mt-5">
            ...
            {loading ? ( // Show loader when loading
                ...
                <div style={{ backgroundColor: '#f5f5f5', padding: '20px', borderRadius: '8px' }}> {/* Light grey background */}
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="d-flex position-relative">
                            <input
                                ...
                                style={{ width: '400px', position: 'relative', overflow: 'hidden', border: '2px solid blue' }}
                            />
                    ...
                    <form onSubmit={handleSaveDetails}>
                        ...
                        <div className="row mb-3">
                            ...
                            <div className="col-md-4 ">
                                <label className="mr-2" style={{ fontWeight: 'bold', textAlign: 'left', display: 'block' }}>Doctor Name</label>
                    ...
                    <div className="mt-4">
                        {slotErrorMessage && <div className="alert alert-danger text-center">{slotErrorMessage}</div>}
                        <h3
                            style={{
                                textAlign: 'center',
                                margin: '32px',
