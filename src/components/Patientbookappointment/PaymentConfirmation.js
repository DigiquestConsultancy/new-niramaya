...
const PaymentConfirmation = () => {
  ...
  const [bookingLoading, setBookingLoading] = useState(false); // Loader for booking process
    ...
    const handlePaymentConfirmation = async () => {
      ...
      try {
        ...
        if (response.data?.status === "SUCCESS") {
          ...
          setBookingLoading(true); // Show loader while booking slot
          const bookingMessage = await bookSlot(amount);
          await patchPatientData();
          setBookingLoading(false); // Stop loader after booking slot
        ...
        <Card.Body className="text-center">
          {loading ? (
            ...
            <>
              ...
              <Button variant="danger" className="mt-3" onClick={() => navigate("/retry-payment")}>
                Retry Payment
              </Button>
            </>
          )}
