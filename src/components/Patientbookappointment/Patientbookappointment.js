...
//       const endpoint = `/clinic/countavailableslots/?doctor_id=3&${datesQuery}`;
...
//         await BaseUrl.get(`/clinic/availableslots/?doctor_id=3&date=${date}`);
//       }
//     } catch (error) {
//       console.error(error);
//     }
...
//     try {
//       const endpoint = `/doctorappointment/blankslot/?doctor_id=3&slot_date=${selectedDate}&consultation_type=${appointmentType}`;
...
//           `/doctorappointment/blankslot/?doctor_id=3&slot_date=${selectedDate}&consultation_type=${type}`
//         );
...
const BookAppointment = () => {
  ...
  const handleCountryChange = (countryName) => {
    ...
    if (country) {
      ...
      const currentNumber = mobile_number?.slice(-10) || "";
      setMobile(`${countryCodes[country.name]}${currentNumber}`);
    }
  };
  ...
  const fetchAvailableSlotsCount = async (selectedDates) => {
    try {
      const datesQuery = selectedDates.map((date) => `dates=${date}`).join("&");
      const endpoint = `/clinic/countavailableslots/?doctor_id=${doctorId}&${datesQuery}`;
      const countResponse = await BaseUrl.get(endpoint);
      const availableCounts = countResponse.data;
      const newSlotCounts = {};
      availableCounts.forEach((count) => {
      ...
      for (let date of selectedDates) {
        await BaseUrl.get(`/clinic/availableslots/?doctor_id=${doctorId}&date=${date}`);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const fetchSlots = async (selectedDate, appointmentType) => {
    try {
      const endpoint = `/doctorappointment/blankslot/?doctor_id=${doctorId}&slot_date=${selectedDate}&consultation_type=${appointmentType}`;
      const slotsResponse = await BaseUrl.get(endpoint);
      const fetchedSlots = slotsResponse.data;
      const { morningSlots, afternoonSlots, eveningSlots } =
        categorizeSlots(fetchedSlots);
  ...
  const handleSlotClick = async (slot, selectedDate, type) => {
    try {
      ...
      if (updateResponse.status === 200) {
        const fetchResponse = await BaseUrl.get(
          `/doctorappointment/blankslot/?doctor_id=${doctorId}&slot_date=${selectedDate}&consultation_type=${type}`
        );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const patientDetails = patientResponse.data?.data;
      const fetchedPatientId = patientDetails?.id;
      const fetchedName = patientDetails?.name;
      const fetchedMobileNumber = patientDetails?.mobile_number;

      localStorage.setItem("patientId", fetchedPatientId);
      localStorage.setItem("patientName", fetchedName);
      localStorage.setItem("patientMobileNumber", fetchedMobileNumber);
      localStorage.setItem("patientAge", age);
      localStorage.setItem("patientBloodGroup", bloodGroup);
      localStorage.setItem("patientGender", gender);
      localStorage.setItem("patientAddress", address);
      ...
      <div className="d-flex justify-content-center mb-5 card-new">
        <Card
          ...
          style={{
            ...
            marginBottom: "50px",
            backgroundColor:
              appointmentType === "clinic" ? "#3D9F41" : "#FFFFFF",
            color: appointmentType === "clinic" ? "#FFFFFF" : "#003F7D",
            border: appointmentType === "clinic" ? "2px solid #3D9F41" : "none",
          }}
          onClick={() => {
            setAppointmentType("clinic");
        ...
        <Card
          ...
          style={{
            ...
            marginBottom: "50px",
            backgroundColor:
              appointmentType === "online" ? "#3D9F41" : "#FFFFFF",
            color: appointmentType === "online" ? "#FFFFFF" : "#003F7D",
            border: appointmentType === "online" ? "2px solid #3D9F41" : "none",
          }}
          onClick={() => {
            setAppointmentType("online");
                ...
                {Array.from({
                  ...
                  >
                    {slots.morning
                      ...
                      .map((slot) => {
                        const currentTime = new Date();
                        const slotDate = new Date(slot.appointment_date);
                        const slotTime = new Date(
                        ...
                        const isDisabled =
                          slot.is_booked ||
                          (isSameDate && currentTime >= slotTime);
                        ...
                        return (
                          <div
                            key={slot.id}
                            onMouseEnter={() =>
                              slot.is_booked && setHoverMessage(" booked")
                            }
                            onMouseLeave={() => setHoverMessage("")}
                            style={{ position: "relative" }}
                          >
