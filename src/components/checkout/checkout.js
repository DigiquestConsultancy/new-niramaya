...
const Checkout = () => {
  ...
  useEffect(() => {
    const fetchFee = async () => {
      try {
        ...
        // Assume country code and consultation type are stored in localStorage
        const countryCode = localStorage.getItem("countryCode") || "91"; // Default to '91' (India)
        const appointmentType = localStorage.getItem("appointmentType") || "clinic"; // Fetch the type of consultation
  
        // Ensure consultation_type is set correctly (either walk-in or online)
        const consultationType =
          appointmentType === "clinic" ? "walk-in" : "online";
  
        // Fetch fee from API with both country code and consultation type
        const response = await BaseUrl.get(
  ...
  const triggerPaymentGateway = async (paymentSessionId) => {
    try {
      ...
      await cashfree.checkout({
        paymentSessionId,
        // returnUrl: "http://localhost:3000/components/paymentconfirmation",
        returnUrl: "https://niramayahomoeopathy.com/components/paymentconfirmation",
      });
    ...
    >
      ...
      >
        ...
        {loading ? (
          ...
          >
            <h3 style={{ marginBottom: "10px" }}>Proceed to Pay</h3>
          </div>
        )}
      ...
      <div className="form-check">
        ...
        <label
          ...
          className="form-check-label"
          style={{
            marginLeft: "8px", // Aligning label after checkbox
          }}
        >
          I agree to the{" "}
          <a href="/Conditions/tearmandcondition" className="text-primary">
      ...
      <div style={{ textAlign: "center", marginTop: "20px" }}>
        <button
          ...
          style={{
            padding: "12px 24px",
            borderRadius: "5px",
            backgroundColor: isChecked ? "#0091A5" : "#ccc", // Disable effect
            color: "white",
            border: "none",
            fontSize: "16px",
            fontWeight: "bold",
            cursor: isChecked ? "pointer" : "not-allowed", // Change cursor style
            width: "100%",
            maxWidth: "300px",
            transition: "background-color 0.3s ease",
          }}
          onMouseOver={(e) => {
            if (isChecked) e.target.style.backgroundColor = "#007A8C";
          }}
          onMouseOut={(e) => {
            if (isChecked) e.target.style.backgroundColor = "#0091A5";
          }}
        >
          PAY
        </button>
